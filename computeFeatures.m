...
addpath(genpath(fullfile(appDirName,'functions')));
...
for i=1:length(data)
    ...
    X(i,20) = SampEn_biores (s_ecgd,1,0.05);
    X(i,21) = FuzzyEn_biores(s_ecgd,3,0.08,2);
...
%%% Save results
save(fullfile(appDirName, 'Features'), 'X');

rmpath(genpath(fullfile(appDirName,'functions')));